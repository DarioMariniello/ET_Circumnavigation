...
agent5_error = select(bag, 'Time', [bag.StartTime bag.EndTime], 'Topic', '/agent5/error');
...
msgse5= readMessages(agent5_error);
...
time_datae3 = zeros(lene3,1);
 ...
 time_datae5 = zeros(lene5,1);
 for i=1:lene5
     time_datae5(i)= double(double(msgse5{i,1}.Header.Stamp.Sec)+double(msgse5{i,1}.Header.Stamp.Nsec)/(10^9))-double(double(msgse1{1,1}.Header.Stamp.Sec)+double(msgse1{1,1}.Header.Stamp.Nsec)/10^9);
 end
...
error5 = zeros(lene5,1);
for i=1:lene5
    error5(i) =double(msgse5{i,1}.Point.X);
end