...
agent5_distance = select(bag, 'Time', [bag.StartTime bag.EndTime], 'Topic', '/agent5/distance');
...
msgsd5= readMessages(agent5_distance);
...
time_datad3 = zeros(lend3,1);
 ...
 time_datad5 = zeros(lend5,1);
 for i=1:lend5
     time_datad5(i)= double(double(msgsd5{i,1}.Header.Stamp.Sec)+double(msgsd5{i,1}.Header.Stamp.Nsec)/(10^9))-double(double(msgsd1{1,1}.Header.Stamp.Sec)+double(msgsd1{1,1}.Header.Stamp.Nsec)/10^9);
 end
...
distance5 = zeros(lend5,1);
for i=1:lend5
    distance5(i) =double(msgsd5{i,1}.Point.X);
end
