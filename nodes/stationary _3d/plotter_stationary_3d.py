...
ZMIN=rp.get_param('zmin')
ZMAX=rp.get_param('zmax')
plt.ion()
plt.figure()
ax=plt.gca(projection='3d')
ax.scatter(*TARGET_POSITION, color=TARGET_COLOR)
ax.axis('equal')
plt.xlim((XMIN,XMAX))
plt.ylim((YMIN,YMAX))
#ax.zlim((ZMIN,ZMAX))
...
def agent_callback(msg, name):
    global agent_positions
    LOCK.acquire()
    agent_positions[name] = [msg.x, msg.y, msg.z]
    LOCK.release()

def estimate_callback(msg, name):
    global agent_estimates
    LOCK.acquire()
    agent_estimates[name] = [msg.x, msg.y, msg.z]
    LOCK.release()
...
while not rp.is_shutdown():
    ...
    for name,pos in ag_pos.items():
        if not pos is None:
            ...
            agent_artists[name] = ax.scatter(*ag_pos[name], color=AGENT_COLOR)
    for name,estim in est.items():
        if not estim is None:
            if not estimate_artists[name] is None:
                estimate_artists[name].remove()
            estimate_artists[name] = ax.scatter(*est[name], color=ESTIMATE_COLOR)
    plt.draw()
    RATE.sleep()
